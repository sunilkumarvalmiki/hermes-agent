...
def test_pairing_dm_policy_group_chat_still_requires_explicit_authorization(monkeypatch):
    """Pairing is DM-only, but open group traffic is still not authorization."""
    _clear_auth_env(monkeypatch)
    config = GatewayConfig(
        platforms={
    ...
    assert runner._is_user_authorized(_source(Platform.WECOM, chat_type="group")) is False
