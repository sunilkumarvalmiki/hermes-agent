...
const pathJoinForPlatform = (platform: NodeJS.Platform): typeof join => (platform === 'win32' ? win32.join : posix.join)
...
export function getVSCodeStyleConfigDir(
  ...
  const pathJoin = pathJoinForPlatform(platform)

  if (platform === 'darwin') {
    return pathJoin(homeDir, 'Library', 'Application Support', appName, 'User')
  }

  if (platform === 'win32') {
    return env['APPDATA'] ? pathJoin(env['APPDATA'], appName, 'User') : null
  }

  return pathJoin(homeDir, '.config', appName, 'User')
}
...
export async function configureTerminalKeybindings(
  ...
  const keybindingsFile = pathJoin(configDir, 'keybindings.json')
...
export async function shouldPromptForTerminalSetup(options?: {
  ...
  try {
    const content = await ops.readFile(pathJoin(configDir, 'keybindings.json'), 'utf8')
    const parsed: unknown = JSON.parse(stripJsonComments(content))
