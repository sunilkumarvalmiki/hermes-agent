...
class WeComAdapter(BasePlatformAdapter):
    ...
    def authorizes_source_via_own_access_policy(self, source) -> bool:
        """Return True only for explicit WeCom adapter-side allowlist matches."""
        ...
        if chat_type == "dm":
            return bool(
                ...
                and _entry_matches(self._allow_from, sender_id)
            )
        ...
        if chat_type in {"group", "forum", "channel"}:
            ...
            if sender_allow:
                return bool(sender_id and _entry_matches(sender_allow, sender_id))

            return bool(
                chat_id
                and self._group_policy == "allowlist"
                and _entry_matches(self._group_allow_from, chat_id)
            )
